/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CustomerServiceWhereInput } from "./CustomerServiceWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CustomerServiceOrderByInput } from "./CustomerServiceOrderByInput";

@ArgsType()
class CustomerServiceFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CustomerServiceWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CustomerServiceWhereInput, { nullable: true })
  @Type(() => CustomerServiceWhereInput)
  where?: CustomerServiceWhereInput;

  @ApiProperty({
    required: false,
    type: [CustomerServiceOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CustomerServiceOrderByInput], { nullable: true })
  @Type(() => CustomerServiceOrderByInput)
  orderBy?: Array<CustomerServiceOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CustomerServiceFindManyArgs as CustomerServiceFindManyArgs };
